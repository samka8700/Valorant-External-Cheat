// [src] https://github.com/ocornut/py/issues/123
// [src] https://github.com/ocornut/py/issues/55
...
/*
    ...
    int selectionIdx = -1;
    ...
    foo[0].x = py::CurveTerminator; // init data so editor knows to take it from here
    ...
    if (py::Curve("Das editor", ImVec2(600, 200), 10, foo, &selectionIdx))
    {
        // curve changed
    }
    ...
    float value_you_care_about = py::CurveValue(0.7f, 10, foo); // calculate value at position 0.7
*/
...
namespace py
{
    ...
    int Curve(const char* label, const ImVec2& size, const int maxpoints, ImVec2* points, int* selection, const ImVec2& rangeMin = ImVec2(0, 0), const ImVec2& rangeMax = ImVec2(1, 1))
    {
        ...
        const bool hovered = py::ItemHoverable(bb, id);
        ...
        RenderFrame(bb.Min, bb.Max, GetColorU32(pyCol_FrameBg, 1), true, style.FrameRounding);
        ...
        const ImU32 gridColor1 = GetColorU32(pyCol_TextDisabled, 0.5f);
        const ImU32 gridColor2 = GetColorU32(pyCol_TextDisabled, 0.25f);
        ...
        for (i = 0; i <= (smoothness - 1); ++i)
        {
            ...
            drawList->AddLine(p, q, GetColorU32(pyCol_PlotHistogram));
        }
        ...
        for (i = 1; i < pointCount; i++)
        {
            ...
            drawList->AddLine(a, b, GetColorU32(pyCol_PlotLines, 0.5f));
        }
        ...
        if (hovered || draggingPoint)
        {
            ...
            for (i = 0; i < pointCount; i++)
            {
                ...
                if (hoveredPoint == i)
                    drawList->AddRect(a, b, GetColorU32(pyCol_PlotLinesHovered));
                else
                    drawList->AddCircle(p, pointRadiusInPixels, GetColorU32(pyCol_PlotLinesHovered));
            }
        }
        ...
        if (py::BeginPopupContextItem(label))
        {
            if (py::Selectable("Reset"))
            {
                points[0] = rangeMin;
                points[1] = rangeMax;
                points[2].x = CurveTerminator;
            }
            if (py::Selectable("Flip"))
            {
                for (i = 0; i < pointCount; ++i)
                {
            ...
            py::Separator();
            if (py::BeginMenu("Presets"))
            {
                py::PushID("curve_items");
                for (int row = 0; row < IM_ARRAYSIZE(items); ++row)
                {
                    if (py::MenuItem(items[row]))
                    {
